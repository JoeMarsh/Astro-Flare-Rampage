    ...
    static class Player
    {
        ...
        public static void ProcessInput()
        {
            if (Ship != null)
            {
                ...
                if (AccelerometerInput.Instance.IsAccelerometerRunning && Config.ControlOption == 3)
                {
                    ...
                    //Ship.Rotation = (float)Math.Atan2(accelerationVector.X, -accelerationVector.Y);
                    Ship.Rotation = (float)Math.Atan2(AccelerometerInput.Instance.AdjustedAccelerometerData.X, -AccelerometerInput.Instance.AdjustedAccelerometerData.Y);
                    directionVector = new Vector2((float)Math.Sin(Ship.Rotation), -(float)Math.Cos(Ship.Rotation));
                    thrustModifier = AccelerometerInput.Instance.AdjustedAccelerometerData.Length();
                    //thrustModifier /= 5;
                    if (thrustModifier > 5)
                        thrustModifier = 5;
