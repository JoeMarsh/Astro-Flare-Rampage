    ...
    class PlayerShip : Ship
    {
        ...
        public override void Update(TimeSpan gameTime)
        {
            ...
            if (this.ProjectileSpeedBar != null)
                this.ProjectileSpeedBar.Percent -= (float)(gameTime.TotalSeconds / 15);

            if (this.AddProjectileBar != null)
                this.AddProjectileBar.Percent -= (float)(gameTime.TotalSeconds / 15);

            if (this.HomingMissileBar != null)
                this.HomingMissileBar.Percent -= (float)(gameTime.TotalSeconds / 15);

            if (this.InvulnAbilityBar != null)
                this.InvulnAbilityBar.Percent -= (float)(gameTime.TotalSeconds / 15);
        ...
        float aimTime;
        public void UpdateAI(TimeSpan gameTime)
        {
            aimTime += (float)gameTime.TotalSeconds;
            if (aimTime > 1)
            {
        ...
        void aiMove()
        {
            ...
            //ability usage
            if (GamePage.playerAbilityUses > 0 && Enemy.Enemies.Count > 30)
            {
                if (Config.ship1Active)
                {
                    if (Player.Ship != null)
